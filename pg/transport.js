...
const { md5AuthMessage, syncMessage, startupMessage, terminateMessage, createParser, getPGError, constants } = require('protocol.js')
...
function setupSocket (sock, config) {
  ...
  function execQuery (sql, onComplete, queries = 1) {
    ...
    const size = 1 + 4 + len + 1 + 1 + 4
    const buf = new ArrayBuffer(size)
    const dv = new DataView(buf)
    let off = 0
    dv.setUint8(off++, 81) // 'Q'
    dv.setUint32(off, 5 + len)
    off += 4
    buf.writeString(sql, off)
    off += len
    dv.setUint8(off++, 0)
    dv.setUint8(off++, 72) // 'H'
    dv.setUint32(off, 4)
  ...
  function terminate (onClose = () => {}) {
    sock.onClose = onClose
    callbacks.push(err => {
      if (err) {
        just.error(err.stack)
      }
      just.print('terminated')
    })
  ...
  sock.terminate = terminate
  sock.flush = flush
  sock.execQuery = execQuery
